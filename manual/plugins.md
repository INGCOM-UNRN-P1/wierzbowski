---
title: "Guía de Extensión y Creación de Plugins: wierzbowski"
subtitle: "Manual de integración, desarrollo de extensiones y uso de la API Python de wierzbowski"
author: "Cátedra de Algoritmos y Programación"
date: "2026-08-31"
---

(manual-wierzbowski-plugins)=
# Guía de Extensión y Plugins: wierzbowski

````{abstract}
Esta guía técnica detalla cómo desarrollar extensiones, crear nuevos plugins e integrar programáticamente **`wierzbowski`** en herramientas de evaluación, entornos de integración continua (CI/CD) o scripts docentes.
````

---

(manual-wierzbowski-plugins-arquitectura)=
## 1. Arquitectura de Extensión

`wierzbowski` provee una arquitectura modular desacoplada basada en puntos de entrada (Entry Points) estándar de Python (`[project.entry-points]`) o interfaces de inyección funcional:

- **Mecanismo de Extensión Principal**: `Extensiones y Reglas Personalizadas para wierzbowski`.
- **Punto de Entrada Oficial**: `wierzbowski.plugins`.
- **Formato de Comunicación**: Estructuras de datos serializables JSON / Pydantic models.

---

(manual-wierzbowski-plugins-tutorial)=
...
class CustomPlugin:
    """Plugin de extensión para wierzbowski."""
    name = "custom_rule"
...
Para que `wierzbowski` descubra y cargue automáticamente tu plugin, agregalo en tu `pyproject.toml`:

````{code-block} toml
[project.entry-points."wierzbowski.plugins"]
mi_plugin = "mi_paquete.modulo:MiPlugin"
````

### Paso 3: Instalar y Verificar el Plugin

Instalá tu extensión en modo editable y comprobá que `wierzbowski` la reconozca:
...
wierzbowski plugins list
````

---

(manual-wierzbowski-plugins-sdk)=
## 3. Conexión Programática mediante la API Python

Podés importar y ejecutar `wierzbowski` directamente desde scripts de Python sin invocar subprocesos:
...
resultado = wierzbowski.ejecutar_analisis(
...
(manual-wierzbowski-plugins-ci)=
## 4. Integración en Pipelines de CI/CD (GitHub Actions / GitLab CI)

Podés integrar `wierzbowski` en tus flujos automatizados de Git para bloquear entregas que no cumplan los requisitos de cátedra:
...
jobs:
  auditoria:
    ...
    steps:
      ...
      - name: Instalar wierzbowski
        run: pip install -e ./wierzbowski
        
      - name: Ejecutar Auditoría
        run: wierzbowski check src/ include/ --json > reporte.json
````

---

(manual-wierzbowski-plugins-ejercicios)=
